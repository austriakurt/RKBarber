...
type BookingEmailStatus = "pending" | "approved" | "cancelled" | "rescheduled" | "completed";
...
async function uploadViaCloudinary(dataUrl: string, fileName: string, folder?: "gcash" | "proofs" | "barbers" | "gallery"): Promise<string> {
...
function getForceCompleteEligibleAtIso(now = new Date()): string {
  const eligibleAt = new Date(now.getTime() + getForceCompleteAfterHours() * 60 * 60 * 1000);
...
function buildBookingStatusEmailHtml(params: {
  ...
  const rows = [
    ...
    .map(([k, v]) => `<tr><td style="padding:8px 0;color:#64748B;font-size:13px">${String(k)}</td><td style="padding:8px 0;text-align:right;font-weight:700;color:#0F172A;font-size:13px">${String(v)}</td></tr>`)
    .join("");
...
export async function registerRoutes(
  ...
  app.get("/api/bookings/action", async (req, res, next) => {
    try {
      const rawAction = String(req.query.action || req.query.decision || "").trim().toLowerCase();
      ...
      if (!bookingId || !token || (action !== "confirm" && action !== "decline" && action !== "complete")) {
        return res.status(400).setHeader("Content-Type", "text/html; charset=utf-8").send(
          actionResultHtml("Invalid Action Link", "This booking action link is incomplete or invalid.", false),
        );
      }
      ...
      if (!bookingSnap.exists) {
        return res.status(404).setHeader("Content-Type", "text/html; charset=utf-8").send(
          actionResultHtml("Booking Not Found", "This booking record no longer exists.", false),
        );
      }
      ...
      if (!storedHash || storedHash !== hashActionToken(token)) {
        return res.status(403).setHeader("Content-Type", "text/html; charset=utf-8").send(
          actionResultHtml("Link Not Valid", "This booking action link has expired or is no longer valid.", false),
        );
      }
      ...
      if (booking.customerActionRequired === false || String(booking.status || "") === "completed") {
        ...
        return res.status(200).setHeader("Content-Type", "text/html; charset=utf-8").send(
          actionResultHtml("Already Processed", alreadyMessage, true),
        );
      }
      ...
      if (isLegacyDecision && String(booking.status || "") !== "pending") {
        return res.status(409).setHeader("Content-Type", "text/html; charset=utf-8").send(
          actionResultHtml("Link Not Applicable", "This link is for a previous booking flow and can no longer be used.", false),
        );
      }

      if (isCompletionAction && String(booking.status || "") !== "confirmed") {
        return res.status(409).setHeader("Content-Type", "text/html; charset=utf-8").send(
          actionResultHtml("Completion Not Available", "This booking is not in a confirmable state for completion.", false),
        );
      }
      ...
      if (toEmail.includes("@")) {
        const emailStatus: BookingEmailStatus = isCompletionAction ? "completed" : action === "confirm" ? "approved" : "cancelled";
      ...
      return res.status(200).setHeader("Content-Type", "text/html; charset=utf-8").send(
        actionResultHtml(
          isCompletionAction ? "Service Completion Confirmed" : action === "confirm" ? "Booking Confirmed" : "Booking Declined",
          isCompletionAction
  ...
  app.patch("/api/admin/bookings/:id", requireAdmin, async (req, res, next) => {
    try {
      ...
      if (parsed.data.status === "completed" && existing.type === "reservation" && existing.customerDecision !== "accepted") {
        ...
        if (existing.customerActionRequired === true && Number.isFinite(eligibleAtMs) && Date.now() < eligibleAtMs) {
          return res.status(409).json({
            message: "Client completion is still pending. Force-complete becomes available after the configured waiting time.",
          });
        }
      }
      if (parsed.data.status === "confirmed" && existing.type === "reservation" && String(existing.status || "") === "completed") {
        return res.status(409).json({ message: "Completed bookings cannot be moved back to confirmed." });
      }
      ...
      if (parsed.data.date || parsed.data.time) {
        ...
        if (toEmail.includes("@")) {
          const completionHtml = buildBookingCompletionRequestEmailHtml({
            ...
            date: String(parsed.data.date || existing.date || ""),
            time: String(parsed.data.time || existing.time || ""),
            price: Number(existing.price || 0),
          });

          let completionEmailResult: { sent: boolean; reason?: string } = { sent: false, reason: "Completion email not sent" };
          try {
            completionEmailResult = await sendBookingActionEmail({
              to: toEmail,
          ...
          updates.emailNotificationSent = completionEmailResult.sent;
          updates.emailNotificationError = completionEmailResult.reason || "";
        } else {
      ...
      if (notificationStatus && toEmail.includes("@")) {
        ...
        const html = buildBookingStatusEmailHtml({
          ...
          previousSchedule: parsed.data.date || parsed.data.time
            ? {
      ...
      return res.status(200).json({ id, ...updates, emailSent: emailResult.sent, emailReason: emailResult.reason || "" });
    } catch (error) {
      return next(error);
    }
  });
  ...
  app.post("/api/admin/queue/call-next", requireAdmin, async (req, res, next) => {
    try {
      ...
      type QueueDoc = { id: string; barberId: string; position: number; status: string; customerName: string; email?: string; phone?: string; queueNotifiedAt?: string };
      ...
      try {
        const barberDoc = await adminDb.collection("barbers").doc(barberId).get();
        if (barberDoc.exists) {
          barberName = String((barberDoc.data() as Record<string, unknown>).name || barberName);
        }
      } catch { /* ignore */ }
      ...
      for (let i = 0; i < Math.min(waitingItems.length, 2); i++) {
        ...
        try {
          ...
          if (result.sent) {
            ...
            await adminDb.collection("queue").doc(customer.id).set(
  ...
  app.post("/api/admin/gallery", requireAdmin, async (req, res, next) => {
    try {
      ...
      if (!data.imageUrl) return res.status(400).json({ message: "imageUrl is required" });
  ...
  app.patch("/api/admin/gallery/:id", requireAdmin, async (req, res, next) => {
    try {
      ...
      if (req.body?.barberId !== undefined) update.barberId = String(req.body.barberId);
      if (req.body?.hairstyleName !== undefined) update.hairstyleName = String(req.body.hairstyleName);
      if (req.body?.caption !== undefined) update.caption = String(req.body.caption);
