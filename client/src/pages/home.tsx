...
import { Clock, CheckCircle2, Scissors, Star } from 'lucide-react';
...
export default function Home() {
  ...
  const lowestPrice =
    positiveWalkinPrices.length > 0 ? Math.min(...positiveWalkinPrices) : 80;
  ...
  return (
    <AmbientPageBackground className="min-h-screen bg-background selection:bg-primary selection:text-primary-foreground">
          ...
          <div className="container mx-auto px-4 md:px-6 mb-10">
            ...
            >
              ...
              <p className="text-muted-foreground">
                Swipe or drag to browse hairstyles groomed by our barbers
              </p>
            </motion.div>
          </div>
              ...
              <CarouselContent className="ml-0 gap-4">
                {(() => {
                  const displayGallery =
                    gallery.length < 10
                      ? Array(Math.ceil(10 / gallery.length))
                          .fill(gallery)
                          .flat()
                      : gallery;
                  return displayGallery.map((item, i) => {
                    ...
                    return (
                      <CarouselItem
                        key={`${item.id}-${i}`}
                        className="pl-0 basis-auto first:pl-4"
                      >
                        <div className="group shrink-0 w-64 h-64 md:w-80 md:h-80 rounded-2xl overflow-hidden border border-border/30 shadow-lg relative cursor-grab active:cursor-grabbing select-none">
                          <img
                            src={item.imageUrl}
                            alt={item.hairstyleName || 'Hairstyle'}
      ...
      <section
        id="about"
        className="py-20 md:py-28 bg-card border-b border-border/30"
      >
        <div className="container mx-auto px-4 md:px-6">
                ...
                {[
                  {
                    ...
                    desc: `${activeBarbers.length} highly trained professionals dedicated to your grooming.`,
                  },
                  {
                    icon: Clock,
                    title: 'Quick Service',
                    desc: 'Efficient bookings and real-time queue tracking so you always know when to arrive.',
                  },
                  {
                    icon: Star,
                    title: 'Affordable Rates',
                    desc: `Premium quality at competitive prices. Starting from ₱${lowestPrice}.`,
                  },
