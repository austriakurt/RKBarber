...
import type { Barber, Booking, QueueItem, Service, ShopSettings, GalleryItem } from "@/lib/types";
...
export async function adminCallNextInQueue(barberId: string): Promise<CallNextResult> {
  const response = await adminRequest("POST", "/api/admin/queue/call-next", { barberId });
...
// Gallery
export async function adminCreateGalleryItem(data: Omit<GalleryItem, "id">): Promise<void> {
  await adminRequest("POST", "/api/admin/gallery", data as Record<string, unknown>);
}
...
export async function adminUpdateGalleryItem(id: string, data: Partial<Omit<GalleryItem, "id">>): Promise<void> {
  await adminRequest("PATCH", `/api/admin/gallery/${id}`, data as Record<string, unknown>);
}
